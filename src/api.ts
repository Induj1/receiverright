...
export function readSession(): Session | null {
  try {
    ...
    return value?.token && value?.workspaceId ? value : null;
  } catch {
    return null;
  }
}
...
export function validateUpload(
  file: File,
  kind: "invoice" | "photo" = "invoice",
) {
  if (file.size < 8 || file.size > 5 * 1024 * 1024)
  ...
  if (!allowed.includes(file.type))
    throw new Error(
      kind === "invoice"
        ? "Choose a JPG, PNG image or PDF."
        : "Choose a JPG or PNG delivery photograph.",
    );
}
...
export async function uploadFile(
  ...
  if (!response.ok)
    throw new Error("Your file did not finish uploading. Please retry.");
  return post<Evidence>(`/uploads/${result.uploadId}/complete`);
}
export async function downloadCase(id: string, format: "json" | "csv") {
  const response = await fetch(`/api/cases/${id}/export?format=${format}`, {
    headers: { Authorization: `Bearer ${readSession()?.token}` },
  });
  if (!response.ok)
    throw new Error("Export could not be downloaded. Please try again.");
  const url = URL.createObjectURL(await response.blob());
  const link = document.createElement("a");
  link.href = url;
  link.download = `receiverright-${id}.${format}`;
  link.click();
