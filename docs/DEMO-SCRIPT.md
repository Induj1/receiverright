...
- The current AWS account returned `SubscriptionRequiredException` for Textract, so extraction is disabled and the current demo uses manual review. Only add a live Textract segment after account activation and a successful real call, using a supported JPEG/PNG/PDF invoice you have permission to use. The bundled sample SVG is an illustration and does not run through Textract.
...
| 0:18–0:35 | Synthetic banner, original invoice, source highlight | “This is labeled synthetic data. The original document stays beside editable item fields. Our Textract adapter is built, but account activation currently blocks extraction, so this demonstration uses manual review.” |
...
| 2:03–2:29 | Architecture plus actual deployed AWS resources/provider indicator | “API Gateway provides HTTPS; Lambda serves the mobile app and API. DynamoDB keeps records and revision checks; private versioned S3 keeps evidence. Textract and Bedrock adapters are built, but account activation currently blocks those services. Manual entry and deterministic summaries keep this workflow available.” |
